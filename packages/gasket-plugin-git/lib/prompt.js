 ...
 * @returns {import("@gasket/cli").CreateContext} context with gitignore
 */
...
module.exports = async function promptHook(gasket, context, { prompt }) {
  if (!('gitInit' in context)) {
    ...
    return instantiateGitignore(Object.assign({}, context, { gitInit }));
  }

  if ('gitInit' in context) {
    return instantiateGitignore(context);
  }
