...
export class LocaleUtils {
   ...
   * @param {string} [config.basePath] - Locale file base path. Defaults to
   * `manifest.basePath`
   * @constructor
   */
  constructor(config: { manifest: LocaleManifest; basePath?: string });
  /**
   * Fallback to the lang part of a locale or to defaultLocale. Strategy is:
   * <locale> <locale lang (if doesn't match default lang)> <default locale (if
   * a locale)> <default lang> null
   *
   * Here's an example using da-DK/da with en-US as defaultLocale da-DK ==> da
   * ==> en-US ==> en ==> null
   *
   ...
   * use in browser.
   *
   * @param {LocalePathPartOrThunk|LocalePathPartOrThunk[]} localePathPart -
