...
describe('mkdir', () => {
  ...
  it('Rejects with message if directory was not allowed to be overwritten', async () => {
    await expect(mkDir({ context: { ...mockContext, extant: true, destOverride: false } })).rejects.toThrow('was not allowed to be overwritten');
  });
  ...
  it('Rejects with original error for other issues', async () => {
    ...
    await expect(mkDir({ context: mockContext })).rejects.toEqual(mockError);
  });
});
