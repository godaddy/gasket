 ...
 */
function generateContent(docsConfigSet, graphs) {
  ...
  const addGraph = graph => { addContent(`### ${graph.name}`); addContent(graph.content); };
 ...
 * @returns {Promise<string>} filename
 */
async function generateIndex(docsConfigSet, graphs) {
  const { docsRoot } = docsConfigSet;

  const target = path.join(docsRoot, 'README.md');
  const content = await generateIndex.generateContent(docsConfigSet, graphs);
