...
async function derivePluginsFromPackage(context) {
  const { dest } = context;
  const pkgPath = path.join(dest, 'package.json');
  const pkg = await readFile(pkgPath, 'utf8');
  const packageJson = JSON5.parse(pkg);
  const dependencies = packageJson.dependencies || {};
  Object.keys(dependencies).forEach((dep) => {
    if (dep.startsWith('@gasket/plugin-')) {
      const importName = dep.split('/')[1].split('-').map((word, index) => {
        if (index === 0) return word;
        return word.charAt(0).toUpperCase() + word.slice(1);
      }).join('');
      context.gasketConfig.addPlugin(importName, dep);
    }
  });
}
 ...
 */
export default async function writeGasketConfig(context) {
  const { dest, gasketConfig, generatedFiles, typescript } = context;
  await derivePluginsFromPackage(context);
